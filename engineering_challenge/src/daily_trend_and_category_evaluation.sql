...
WHERE
   ...
   TO_DATE(TRY_TO_TIMESTAMP_NTZ("requested_datetime")) >= DATEADD('days',-5,CURRENT_DATE()) -- use for incremental addition
GROUP BY "date";

--- by closed cases
CREATE OR REPLACE TABLE "damaged_property_closed_cases" AS
SELECT
   TO_DATE(TRY_TO_TIMESTAMP_NTZ("closed_date")) as "date",
   count(*) as "closed_cases_count"
FROM
   "san_francisco_data"
WHERE
   "service_name" = 'Damaged Property' AND
   TO_DATE(TRY_TO_TIMESTAMP_NTZ("closed_date")) >= DATEADD('days',-5,CURRENT_DATE())
GROUP BY "date";
...
SELECT
  ...
  IFF(t2."closed_cases_count" = 0 OR t2."closed_cases_count" IS NULL, 0,  (t1."closed_cases_count" / t2."closed_cases_count"-1)*100) as "closed_cases_daily_percentage_growth"
FROM 
